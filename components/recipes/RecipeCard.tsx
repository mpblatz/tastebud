...
export function RecipeCard({ recipe }: { recipe: DatabaseRecipe }) {
    return (
        <Link href={`/recipes/${recipe.id}`} className="no-underline group">
            <div className="flex flex-col md:flex-col max-md:flex-row rounded-md border border-[var(--border)] bg-card shadow-card transition-all duration-300 group-hover:shadow-card-hover group-hover:border-[var(--border-hover)] overflow-hidden">
                ...
                <div className="flex-1 min-w-0 px-5 py-[18px] max-md:px-4 max-md:py-[14px] flex flex-col gap-1.5">
                    <h3 className="line-clamp-2 font-heading font-bold tracking-[-0.02em] text-foreground">
                        {recipe.title}
                    </h3>
                    ...
                    <div className="flex items-center gap-2 overflow-hidden">
                        {recipe.import_url && (
                            <span className="text-[11px] font-mono text-text-faint tracking-[0.02em] truncate shrink-0">
                            ...
                            <div className="flex gap-1.5 overflow-hidden">
                                {recipe.recipes_tags.map((tag) => {
                                    ...
                                    return (
                                        <span
                                            key={tag.tag.id}
                                            className="text-[10px] font-mono tracking-[0.03em] rounded-sm px-[7px] py-[2px] whitespace-nowrap shrink-0"
                                            style={{
                                                color,
